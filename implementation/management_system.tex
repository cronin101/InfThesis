...
Luckily, at the point that the branch is possibly triggered, no mathematical operators will mutate the previous value directly. This means that the difficultly of handling branching can be sidestepped by simply leaving an expression ending in a conditional and starting a new stack. All expressions produced are then simply combined in order after the token stream has been exhausted.
